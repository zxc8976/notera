...
_TAG_LIST = r"(?:span|spanclass|div|p|a|code|pre|br|strong|em|b|i|u|mark|table|thead|tbody|tr|td|th|ul|ol|li|figure|figcaption|section|article|header|footer|nav|math|mrow|mstyle|mtext|mfrac|msup|msub|mtable|mi|mo|mn)"
...
def _is_repetitive_noise_line(line: str) -> bool:
    ...
    if re.search(r"(Math){3,}", s):
        return True
    if re.search(r"パララ{3,}", s):
        return True
...
def _repair_image_markdown(text: str) -> str:
    if not text:
        return ""
    fixed = re.sub(r"(!\[[^\]]*\]\([^\)\n]+)(?=\s*(\n|$))", r"\1)", text)
    # Some legacy generators emit image links without leading "!".
    # Promote only obvious image links under /images or common image extensions.
    fixed = re.sub(
        r"(?m)^(\s*)\[([^\]]*)\]\(([^\)\n]*(?:/images/|\.(?:png|jpe?g|webp|gif|bmp))[^\)\n]*)\)",
        r"\1![\2](\3)",
        fixed,
    )
    return fixed
...
def clean_markdown(text: str) -> str:
    ...
    working = _repair_image_markdown(working)
